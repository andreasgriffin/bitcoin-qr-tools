...
import click
from bbqr import join_qrs, split_qrs
...
@click.group()
@click.option("--pdb", is_flag=True, help="Prepare patient for surgery to remove bugs.")
def main(**kws):
...
@click.option(
    ...
    metavar="(char)",
    default=None,
    type=click.Choice("H2Z"),
    help="Force low-level encoding: H 2 or Z",
)
@click.option(
    "--filetype",
    "-t",
    metavar="(char)",
    default=None,
    type=click.Choice(KNOWN_FILETYPES),
    help="Force specific file type code: " + "".join(KNOWN_FILETYPES),
)
@click.option(
    "--max-version",
    "-v",
...
@click.option(
    "--min-split", "-m", metavar="NUM", default=1, help="Produce at least this many QR codes (default: 1)"
)
@click.option(
    "--frame-delay",
    "-d",
...
def make_qrs(
    ...
    vers, parts = split_qrs(
        raw, type_code=filetype, encoding=encoding, max_version=max_version, min_split=min_split
    )
    ...
    qs = [pyqrcode.create(data, error="L", version=vers, mode="alphanumeric") for data in parts]
        ...
        for i in range(num_parts):
            ...
            if num_parts > 1:
                ...
                for j in range(num_parts):
                    draw.rectangle(
                        (lm + (j * pw), y, lm + ((j + 1) * pw), y + h), fill=(128 if i != j else 0)
                    )
